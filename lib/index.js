...
function injectBase(html, port, backendPorts) {
  ...
  // Import map: remap absolute URLs so ES module imports go through the proxy
  // import "/@vite/client" → import "/preview/PORT/@vite/client"
  const importMap = `<script type="importmap">{"imports":{"/@vite/":"${prefix}@vite/","/@id/":"${prefix}@id/","/@fs/":"${prefix}@fs/","/src/":"${prefix}src/"}}</script>`;
