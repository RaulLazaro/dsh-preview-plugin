    ...
    try {
      return ctx.webServer.register({
        ...
        handler: async (req, res) => {
          ...
          const isAsset = /\.\w{2,10}$/.test(lastSegment) &&
            !/\.html?$/i.test(lastSegment);

          try {
            if (isAsset) {
              // Static asset: proxy directly from the requested path
              ...
              // SPA navigation: always serve root HTML so client-side router handles routing
              const rootUrl = `http://127.0.0.1:${port}/`;
              const upstream = await proxyFetch(rootUrl, port);
              const contentType = upstream.headers.get('content-type') ?? 'application/octet-stream';
