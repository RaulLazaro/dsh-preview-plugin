...
function loadState() {
  try {
    ...
    if (raw && typeof raw === 'object') {
      globalPort = raw.port === null || raw.port === undefined ? null : String(raw.port);
      globalBackendPorts = Array.isArray(raw.backendPorts) ? raw.backendPorts.map(String) : [];
      if (globalPort) portBackendPorts.set(globalPort, globalBackendPorts);
    }
  } catch {
